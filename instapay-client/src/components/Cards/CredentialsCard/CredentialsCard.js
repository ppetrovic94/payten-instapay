import React from 'react';
import PropTypes from 'prop-types';
import { Card, Button, Image } from 'semantic-ui-react';
import CustomModal from '../../CustomModal/CustomModal';
import './CredentialsCard.scss';

const CredentialsCard = ({
  details,
  terminalUserId,
  merchantUserId,
  noCredentials,
  regenerateCredentials,
  deleteCredentials,
  fetchTerminalById,
}) => {
  const credentialsModalContent = () => (
    <>
      <h3>{`Generisanje novih kredencjiala će vratiti terminal ${details.acquirerTid} u 'Inactive' status nakon čega će terminal zahtevati ponovnu aktivaciju sa novim kredencijalima.
        `}</h3>
      <h3>{`Da li ste sigurni da želite da generišete nove kredencijale za ${details.acquirerTid} ?`}</h3>
    </>
  );

  const generateCredentialsContent = () => (
    <>
      <h3>{`Da li ste sigurni da želite da generišete kredencijale za ${details.acquirerTid} ?`}</h3>
    </>
  );

  const deleteCredentialsContent = () => (
    <>
      <h3>{`Brisanjem kredencijala možete onemogućiti rad terminala.
        `}</h3>
      <h3>{`Da li ste sigurni da želite da obrišete kredencijale za ${details.acquirerTid} ?`}</h3>
    </>
  );

  return (
    <Card
      style={{
        width: '400px',
        marginRight: '20px',
      }}
      color="red">
      <h3 className="terminalDetailsUserId">User ID (scan)</h3>
      {noCredentials && (
        <div style={{ display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
          <p style={{ color: 'red' }}>Terminal nema generisane kredencijale.</p>
          <CustomModal
            content={generateCredentialsContent}
            yesNoButtons
            onAcceptHandler={async () => {
              await regenerateCredentials(details.terminalId);
              await fetchTerminalById(details.terminalId);
            }}
            triggerElement={() => <Button color="teal">Generiši kredencijale</Button>}
          />
        </div>
      )}
      {merchantUserId && (
        <div style={{ display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
          <p style={{ color: 'red' }}>Kredencijali su generisani na nivou trgovca.</p>
          <Image
            src={`http://localhost:8080/ips/api/user/terminals/qrcode/${merchantUserId}`}
            style={{
              width: '200px',
            }}
            wrapped
          />
        </div>
      )}
      {terminalUserId && (
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <Image
            src={`http://localhost:8080/ips/api/user/terminals/qrcode/${terminalUserId}`}
            style={{
              width: '200px',
            }}
            wrapped
          />
        </div>
      )}
      <Card.Content>
        <div className="terminalDetailsCardContent">
          <h3>TID:</h3>
          <p className="terminalDetailsData">{details.acquirerTid}</p>
        </div>
        <div className="terminalDetailsCardContent">
          <h3>Status:</h3>
          <p className="terminalDetailsData">{details.status.statusName}</p>
        </div>
        <div className="terminalDetailsCardContent">
          <h3>Aktivacioni kod:</h3>
          <p className="terminalDetailsData">
            {merchantUserId || noCredentials ? '/' : details.activationCode}
          </p>
        </div>
        <div className="terminalRegenerate">
          {terminalUserId && (
            <>
              <CustomModal
                content={credentialsModalContent}
                yesNoButtons
                onAcceptHandler={async () => {
                  await regenerateCredentials(details.terminalId);
                  await fetchTerminalById(details.terminalId);
                }}
                triggerElement={() => <Button color="teal">Generiši nove kredencijale</Button>}
              />
              <CustomModal
                content={deleteCredentialsContent}
                yesNoButtons
                onAcceptHandler={async () => {
                  await deleteCredentials(terminalUserId);
                }}
                triggerElement={() => <Button color="red">Obriši kredencijale</Button>}
              />
            </>
          )}
        </div>
      </Card.Content>
    </Card>
  );
};

CredentialsCard.propTypes = {
  details: PropTypes.object,
  terminalUserId: PropTypes.string,
  merchantUserId: PropTypes.string,
  noCredentials: PropTypes.bool,
  regenerateCredentials: PropTypes.func,
  deleteCredentials: PropTypes.func,
  fetchTerminalById: PropTypes.func,
};

export default CredentialsCard;
